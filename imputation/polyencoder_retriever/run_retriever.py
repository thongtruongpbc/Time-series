...
import polyencoder_retriever

import torch
import torch.backends
from polyencoder_retriever.exp.exp_retriever import Exp_Retriever
from polyencoder_retriever.visualization.Pearson import Retriever_visualization
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Number of Retrievals when building dataset for retrieval",
    )
    ...
    if args.task_name == "retriever":
        Exp = Exp_Retriever
    if args.task_name == "visualization":
        Exp = Retriever_visualization
    if args.is_training:
        for ii in range(args.itr):
            ...
            if args.task_name == "retriever":
                with mlflow.start_run(run_name=f"Run_{setting}") as run:
