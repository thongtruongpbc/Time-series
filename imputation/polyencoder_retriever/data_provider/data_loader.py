...
import re
import torch
from polyencoder_retriever.utils.multivariate_stl import Multivariate_STL
...
class Dataset_ETT_hour(Dataset):
    def __init__(
        ...
        self.revin = RevIN(num_features=self.args.enc_in, affine=False)
        self.__read_data__()
    ...
    def __read_data__(self):
        ...
        # STL decomposition
        self.data_stl = Multivariate_STL(self.data_x)
        self.pos_view = self.data_stl
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.revin(seq_x, mode="norm").detach().cpu().numpy()
        T, C = seq_x.shape
        context_mask = torch.rand((T, C))
        pos_candidate_mask = torch.rand((T, C))
        # context
        context_mask[context_mask <= self.args.mask_rate] = 0  # masked
        context_mask[context_mask > self.args.mask_rate] = 1  # remained
        context = np.where(context_mask == 0, 0, seq_x)
        context = torch.from_numpy(context).float()
        context = (
            self.revin(context, mode="norm", mask=context_mask).detach().cpu().numpy()
        )

        # positive candidate
        pos_candidate = self.pos_view[s_begin:s_end]
        pos_candidate = torch.from_numpy(pos_candidate).float()
        pos_candidate = self.revin(pos_candidate, mode="norm").detach().cpu().numpy()

        seq_y = self.data_y[r_begin:r_end]
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
        return index, context, seq_x, pos_candidate, seq_x_mark, context_mask
...
class Dataset_ETT_minute(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_x_raw = torch.from_numpy(self.data_x[s_begin:s_end]).float()
        seq_x = self.revin(seq_x_raw, mode="norm").detach().cpu().numpy()

        T, C = seq_x.shape
        context_mask = torch.rand((T, C))
        context_mask = (context_mask > self.args.mask_rate).float()

        context = np.where(context_mask == 0, 0, seq_x)
        context = (
            self.revin(
                torch.from_numpy(context).float(), mode="norm", mask=context_mask
            )
            .detach()
            .cpu()
            .numpy()
        ...
        pos_candidate = torch.from_numpy(self.pos_view[s_begin:s_end]).float()
        pos_candidate = self.revin(pos_candidate, mode="norm").detach().cpu().numpy()
...
class UEAloader(Dataset):
    ...
    def _resolve_ts_path(self, root_path, dataset_name, flag):
        ...
        return hf_hub_download(
            HUGGINGFACE_REPO, filename=f"{dataset_name}/{fname}", repo_type="dataset"
        )
