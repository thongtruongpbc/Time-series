...
fmt_bytes = (
    lambda v: str(v >> ((max(v.bit_length() - 1, 0) // 10) * 10))
    + ["", "K", "M", "G", "T", "P", "E"][max(v.bit_length() - 1, 0) // 10]
    + "iB"
)
...
def plot_averages(path, x, y, ranks):
    ...
    try:
        ...
        print(
            "!!! Can't generate plot. Please run `pip install matplotlib` to enable plotting. !!!\n"
        )
        return
...
def plot_profile(path, y, ranks):
    ...
    plt.ylabel("Bus bandwidth (GBps)")
    plt.title(
        f"all-reduce bus bandwidth profile for {args.payload_size_in_gib}GiB payload on ranks={ranks}"
    )
    # plt.xticks(rotation=45)
...
def run(local_rank):
    ...
    def finish():
        ...
        print(f"\nEnvironment:")
        print(
            f"- software: torch={torch.__version__}, cuda={torch.version.cuda}, nccl={torch.cuda.nccl.version()}"
        )
        print(f"- hardware: {get_device_info()}\n")

        if args.profile_stability:
            print(
                f"The {args.payload_size_in_gib}GiB payload bandwidth of all_reduce over {ranks} ranks ({args.num_warmup_iterations} warmups / {args.num_iterations} trials):\n"
            )
            print(f"|    busbw   |    algbw   |")
            print(f"| ---------: | ---------: |")
            for i in range(len(busbw_points)):
                print(
                    f"| {conv_to_GBps(busbw_points[i]):6.2f}GBps | {conv_to_GBps(algbw_points[i]):6.2f}GBps |"
                )
            ...
            print(
                f"The average bandwidth of all_reduce over {ranks} ranks ({args.num_warmup_iterations} warmups / {args.num_iterations} trials):\n"
            )
            print(f"| payload |    busbw   |    algbw   |")
            print(f"| ------: | ---------: | ---------: |")
            for size in busbw.keys():
                print(
                    f"| {fmt_bytes(size):>7} | {conv_to_GBps(busbw[size]):6.2f}GBps | {conv_to_GBps(algbw[size]):6.2f}GBps |"
                )
...
def device_id_kwargs(local_rank):
    ...
    if "device_id" in inspect.signature(
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="The number of iterations used to benchmark each collective call",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="payload size in GiBs, e.g. 4 (4GiB). If not specified the full range 2**15 .. 2**32 will be benchmarked",
    )
    parser.add_argument(
        "--profile_stability",
        action="store_true",
        help="Reports individual results for each non-warmup iteration. Requires --payload_size_in_gib. This is used to test the stability of performance, rather than reporting an averaged outcome.",
    )
    ...
    if args.profile_stability and args.payload_size_in_gib is None:
        raise ValueError(
            "--profile_stability requires --payload_size_in_gib to profile one specific payload, e.g. to profile a 0.5GiB payload use: --profile_stability --payload_size_in_gib 0.5"
        )
