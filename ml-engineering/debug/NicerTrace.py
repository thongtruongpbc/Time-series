...
class NicerTrace(trace.Trace):
    ...
    def __init__(self, *args, packages_to_include=None, log_pids=False, **kwargs):
        ...
        if ignoredirs is not None and len(ignoredirs) > 1:
            if packages_to_include is not None:
                raise ValueError(
                    "can't have both ignoredirs and packages_to_include not None"
                )
            ...
            kwargs["ignoredirs"] = (
                self.stdlib_dir
            )  # everything including python core packages
        else:
            ...
            dirs_to_exclude = [
                f"{self.site_packages_dir}/{dir}"
                for dir in sorted(packages_to_exclude)
                if not dir.endswith("-info")
            ]
    ...
    def localtrace_trace_and_count(self, frame, why, arg):
        ...
        if why == "line":
            ...
            print(
                f"{basename}:{lineno:>6}: {trace.linecache.getline(filename, lineno)}",
                end="",
            )
        return self.localtrace
