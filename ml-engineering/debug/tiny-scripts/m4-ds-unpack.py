...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="source dataset_name_or_path",
)
parser.add_argument(
    "--target_path",
    required=False,
...
parser.add_argument(
    ...
    default="0",
    type=str,
    help="which ids to extract. example: 1,2,5-7,10",
)
...
def dump_example_shapes(idx, row):
    ...
    imgs_summary = ", ".join(
        [
            f"{v} {k}"
            for k, v in sorted(imgs.items(), key=lambda x: int(x[0].split("x")[0]))
        ]
    )
    ...
    for txt in row["texts"]:
        if txt is None:
            ...
            txts[shape] += 1
    txts_summary = ", ".join(
        [f"{v} {k}" for k, v in sorted(txts.items(), key=lambda x: int(x[0]))]
    )
