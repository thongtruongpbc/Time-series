...
def get_torch_dtype(dtype_str):
    ...
    try:
        ...
        raise ValueError(
            f"Unsupported dtype: {dtype_str}. Must be a valid torch dtype name."
        )
...
class MPSEvent:
    ...
    def elapsed_time(self, other):
        if self._timestamp is None or other._timestamp is None:
            raise RuntimeError(
                "Attempted to measure elapsed time before events were recorded"
            )
...
class MPSArch(Arch):
    ...
    def compute_info(self):
        ...
        if hasattr(torch.backends, "mps") and hasattr(
            torch.backends.mps, "driver_version"
        ):
            try:
...
def benchmark_mm(m, n, k, dtype, device, num_iterations, num_warmup_iterations):
    ...
    l2_cache = torch.empty(
        int(l2_cache_size_in_mbs * 2**20 / 4), dtype=torch.int, device=device
    )
    ...
    if dtype in fp8_dtypes:
        ...
        if dtype == torch.float8_e4m3fn and arch.name == "rocm":
            raise ValueError(
                "ROCm doesn't support float8_e4m3fn, use --dtype float8_e4m3fnuz instead"
            )
...
def setup_checks():
    if arch.name == "rocm":
        if int(os.environ.get("PYTORCH_TUNABLEOP_ENABLED", "0")) == 0:
            warn(
                "AMD GPUs usually require `export PYTORCH_TUNABLEOP_ENABLED=1` to measure the best possible compute, but it hasn't been set. Proceeding as is - expect potentially bad/invalid results."
            )
...
"""
    ...
    print(
        f"Warming up the accelerator for {accelerator_warmup_seconds} secs ... ",
        end="",
        flush=True,
    )
    ...
    for M in m:
        for N in n:
            for K in k:
                num_shapes += 1
                mean_tflops, median_tflops, max_tflops = benchmark_mm(
                    M,
                    N,
                    K,
                    dtype,
                ...
                print(
                    f"{num_shapes:>6} | {mean_tflops:6.1f}(mean) {median_tflops:6.1f}(median) {max_tflops:6.1f}(max) @ {cur_config:<20} | best: {best_tflops['mean']:6.1f}(mean) {best_tflops['median']:6.1f}(median) {best_tflops['max']:6.1f}(max) TFLOPS",
                    end="\r",
                )
    finish()
